...
** root -q -b fitMass.C
*/
void fitMass(int year=2018, TString pmode = "Associated", TString suffix = "redTree"){
    gStyle->SetOptStat(0);
    ...
    TString all_pmodes[4]={"ttH", "WminusH", "WplusH", "ZH"};

    for (int j =0;j<3;j++){
        ofstream outmass;
        outmass.open(Form("sim_massParam_%s_%s_%s_%d.txt",pmode.Data(), suffix.Data(), chanName[j].Data(),year));
        ...
        for (int i =0;i<ntot;i++){
            ...
            if(pmode != "Associated") {
                ...
                for(int pm=0; pm<4; pm++){
                    t->Add(Form("redTrees/%s%d_%s_%d.root",all_pmodes[pm].Data(),mass[i],suffix.Data(),year));
                }
            }
        ...
        RooRealVar* frac_0 = new RooRealVar("frac_0","",0.2,0.,1);
        RooRealVar* frac_1 = new RooRealVar("frac_1","",0.9,0.,1);
        ...
        for (int i=0;i<ntot;i++){
            ...
            addpdf[i] = new RooAddPdf(Form("apdf_%d",i),"",RooArgList(*cpdf[i],*pdf_landau),RooArgList(*frac1));
            cout<< data_stage_cat[i]->sumEntries()<<endl;
            
            // Additional Landau contribution for WH and ZH
            if (pmode.Contains("Wplus")|| pmode.Contains("WminusH") || pmode.Contains("ZH") || pmode.Contains("ttH") || (pmode == "Associated"))
            ...
            /*
            ** An initial guess of the best fit values p0i(mH) is obtained from a fit of the reconstructed m4l 
            ** for fdCB(m4l|mH = 125 GeV) for all the parameters, which are in turn used as input to perform a 
            ** simultaneous fit over the five mass points and derive the expression of pi(mH) used for the analysis
            ** The final mean and sigma of the dCB are taken from mH=125 GeV.
            */
            if(i==0){
                ...
                RooAddPdf *addpdftmp = new RooAddPdf(Form("apdf_%d",i),"",RooArgList(*cpdftmp,*pdf_landautmp),RooArgList(*frac1));
                if (pmode.Contains("Wplus")|| pmode.Contains("WminusH") || pmode.Contains("ZH") || pmode.Contains("ttH"))
        ...
        // Simultaneous fit 
        // Hessian inversion is performed
        simPdf.fitTo(*data_all,SumW2Error(kFALSE),InitialHesse(true),Strategy(2)) ;

        TString a1outs= Form ("a1_%s_%d \t %.6f+%.5f*(MH-125)",chanName[j].Data(),year,a1_0->getVal(),a1_1->getVal());
        TString a2outs= Form ("a2_%s_%d \t %.6f+%.5f*(MH-125)",chanName[j].Data(),year,a2_0->getVal(),a2_1->getVal());
        TString n1outs= Form ("n1_%s_%d \t %.6f+%.5f*(MH-125)",chanName[j].Data(),year,n1_0->getVal(),n1_1->getVal());
        TString n2outs= Form ("n2_%s_%d \t %.6f+%.5f*(MH-125)",chanName[j].Data(),year,n2_0->getVal(),n2_1->getVal());
        TString meanouts= Form ("mean_%s_%d \t %.6f+%.5f*(MH-125)",chanName[j].Data(),year,mean_0->getVal(),mean_1->getVal());
        TString sigmaouts= Form ("sigma_%s_%d \t %.6f+%.5f*(MH-125)",chanName[j].Data(),year,sigma_0->getVal(),sigma_1->getVal());
        ...
        if (pmode.Contains("Wplus")|| pmode.Contains("WminusH") || pmode.Contains("ZH") || pmode.Contains("ttH") ||  (pmode == "Associated")){
            outmass<<Form ("mean_l_%s_%d \t %.6f+%.5f*(MH-125)",chanName[j].Data(),year,mean_l_0->getVal(),mean_l_1->getVal())<<endl;
            outmass<<Form ("sigma_l_%s_%d \t %.6f+%.5f*(MH-125)",chanName[j].Data(),year,sigma_l_0->getVal(),sigma_l_1->getVal())<<endl;
            outmass<<Form ("frac_%s_%d \t %.6f",chanName[j].Data(),year,frac_1->getVal())<<endl;
        }

        /* The final signal lineshape is plotted here.
        ** Note that these plots do not correspond to the pdf obtained from 
        ** a fit of dCB at a given mH value. They are the projection of the
        ** pdf obtained from the simultaneous over a given mH "axis".
        */
        for (int i =0;i<ntot;i++){
            ...
            gPad->Print(Form("simFit_%s_%s_%d_%s_%d.png",pmode.Data(),suffix.Data(),mass[i],chanName[j].Data(),year));
        }
        outmass.close();
    }
}
